...
def coin_change(nums: List[int]) -> int:
    """
    Main solution function for coin change.
    ...
    return coin_change_optimized(nums)


def coin_change_brute_force(nums: List[int]) -> int:
    """
    Brute force approach for coin change.
...
def coin_change_optimized(nums: List[int]) -> int:
    """
    Optimized approach for coin change.
