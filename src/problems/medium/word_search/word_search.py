...
def word_search(nums: List[int]) -> int:
    """
    Main solution function for word search.
    ...
    return word_search_binary_search(nums)


def word_search_brute_force(nums: List[int]) -> int:
    """
    Brute force approach for word search.
...
def word_search_binary_search(nums: List[int]) -> int:
    """
    Optimized approach for word search.
